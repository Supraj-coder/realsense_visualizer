...
class CameraConfig:
    ...
    @property
    def color_shape(self): return (0, self.height, self.width, self.color_channels)
    @property
    def color_chunks(self): return (self.chunk_size_frames, self.height, self.width, self.color_channels)
    
    @property
    def depth_shape(self): return (0, self.height, self.width)
    @property
    def depth_chunks(self): return (self.chunk_size_frames, self.height, self.width)
    
    @property
    def rot_shape(self): return (0, self.quat_components)
    @property
    def rot_chunks(self): return (self.meta_chunk_size, self.quat_components)
...
def main():
    ...
    if args.mode == "record":
        ...
        color_ds = root.create_dataset('color', shape=cfg.color_shape, chunks=cfg.color_chunks, dtype='uint8', compressor=cfg.compressor)
        depth_ds = root.create_dataset('depth', shape=cfg.depth_shape, chunks=cfg.depth_chunks, dtype='uint16', compressor=cfg.compressor)
        ...
        root.attrs.update({
            'intrinsics': {'width': intr.width, 'height': intr.height, 'fx': intr.fx, 'fy': intr.fy, 'ppx': intr.ppx, 'ppy': intr.ppy},
        ...
        with zarr.open(str(path), mode='r') as root:
            if 'timestamp' in root and len(root.timestamp) > 0:
                ...
                for i, timestamp in enumerate(root.timestamp):
                    ...
                    if 'rotation' in root:
                        rr.log("world/camera", rr.Transform3D(rotation=rr.components.RotationQuat(xyzw=root.rotation[i])))
